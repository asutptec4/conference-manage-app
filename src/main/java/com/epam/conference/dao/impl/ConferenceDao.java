...
/**
 * DAO class used for working with {@code Conference} objects and modifying data
 * in corresponding table of database.
 * 
...
public class ConferenceDao extends AbstractDao<Conference> {
    ...
    private static final String FIND_BY_ID = "SELECT id, name, startdate, enddate, location, description FROM conference WHERE id = ?";
	...
	try (PreparedStatement statement = connection
	    ...
	    throw new ConferenceAppDaoException("Can't add conference to db.",
		    e);
	}
    }
	...
	try (PreparedStatement statement = connection
	    ...
	    throw new ConferenceAppDaoException(
		    "Can't delete conference from db.", e);
	}
    }
	...
	try (PreparedStatement statement = connection
	    ...
	    if (resultSet.next()) {
		conference = createConference(resultSet);
	    }
	} catch (SQLException e) {
	    throw new ConferenceAppDaoException("Can't find conference by id.",
		    e);
	}
	...
	try (PreparedStatement statement = connection
		...
		list.add(createConference(resultSet));
	    }
	} catch (SQLException e) {
	    throw new ConferenceAppDaoException(
		    "Can't find any conferences in db.", e);
	}
	return list;
    }
	...
	try (PreparedStatement statement = connection
	    ...
	    throw new ConferenceAppDaoException(
		    "Can't update conference record in db.", e);
	}
    }

    private Conference createConference(ResultSet resultSet)
	    throws SQLException {
